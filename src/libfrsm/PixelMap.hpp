#ifndef __FRSM_PIXEL_MAP_HPP__
#define __FRSM_PIXEL_MAP_HPP__
...
public:
  ...
  PixelMap<T>(const frsm_pixel_map_t * _msg);
  ...
  const frsm_pixel_map_t *get_pixel_map_t(int64_t utime);
  void set_from_pixel_map_t(const frsm_pixel_map_t * _msg);
...
static frsm_pixel_map_t * load_pixel_map_t_from_file(const std::string & name);
#endif
