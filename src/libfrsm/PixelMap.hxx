 ...
 */
template<typename T>
PixelMap<T>::PixelMap(const frsm_pixel_map_t * _msg) :
    msg(NULL), data(NULL)
{
...
#ifndef NO_LCM
template<typename T>
const frsm_pixel_map_t * PixelMap<T>::get_pixel_map_t(int64_t utime)
{
  if (msg == NULL)
    msg = (frsm_pixel_map_t*) calloc(1, sizeof(frsm_pixel_map_t));
  ...
  if (type == typeid(float))
    msg->data_type = FRSM_PIXEL_MAP_T_TYPE_FLOAT;
  else if (type == typeid(uint8_t))
    msg->data_type = FRSM_PIXEL_MAP_T_TYPE_UINT8;
  else if (type == typeid(double))
    msg->data_type = FRSM_PIXEL_MAP_T_TYPE_DOUBLE;
  else if (type == typeid(int32_t))
    msg->data_type = FRSM_PIXEL_MAP_T_TYPE_INT32;
  else if (type == typeid(uint32_t))
    msg->data_type = FRSM_PIXEL_MAP_T_TYPE_UINT32;
  else if (type == typeid(int16_t))
    msg->data_type = FRSM_PIXEL_MAP_T_TYPE_INT16;
  else if (type == typeid(uint16_t))
    msg->data_type = FRSM_PIXEL_MAP_T_TYPE_UINT16;
  else if (type == typeid(int8_t))
    msg->data_type = FRSM_PIXEL_MAP_T_TYPE_INT8;
  else
    msg->data_type = FRSM_PIXEL_MAP_T_TYPE_UNKNOWN;
...
template<typename T>
void PixelMap<T>::set_from_pixel_map_t(const frsm_pixel_map_t * _msg)
{
  ...
  if (_msg->data_type > 0) {
    const std::type_info& type = typeid(T);
    if (_msg->data_type == FRSM_PIXEL_MAP_T_TYPE_FLOAT && type != typeid(float)) {
      fprintf(stderr, "message has %d, not float data, not setting pixmap!\n", _msg->data_type);
      return;
    }
    else if (_msg->data_type == FRSM_PIXEL_MAP_T_TYPE_UINT8 && type != typeid(uint8_t)) {
      fprintf(stderr, "message has %d, not uint8 data, not setting pixmap!\n", _msg->data_type);
      return;
    }
    else if (_msg->data_type == FRSM_PIXEL_MAP_T_TYPE_DOUBLE && type != typeid(double)) {
      fprintf(stderr, "message has %d, not double data, not setting pixmap!\n", _msg->data_type);
      return;
    }
    else if (_msg->data_type == FRSM_PIXEL_MAP_T_TYPE_INT32 && type != typeid(int32_t)) {
      fprintf(stderr, "message has %d, not int32 data, not setting pixmap!\n", _msg->data_type);
      return;
    }
    else if (_msg->data_type == FRSM_PIXEL_MAP_T_TYPE_UINT32 && type != typeid(uint32_t)) {
      fprintf(stderr, "message has %d, not uint32 data, not setting pixmap!\n", _msg->data_type);
      return;
    }
    else if (_msg->data_type == FRSM_PIXEL_MAP_T_TYPE_INT16 && type != typeid(int16_t)) {
      fprintf(stderr, "message has %d, not int16 data, not setting pixmap!\n", _msg->data_type);
      return;
    }
    else if (_msg->data_type == FRSM_PIXEL_MAP_T_TYPE_UINT16 && type != typeid(uint16_t)) {
      fprintf(stderr, "message has %d, not uint16 data, not setting pixmap!\n", _msg->data_type);
      return;
    }
    else if (_msg->data_type == FRSM_PIXEL_MAP_T_TYPE_INT8 && type != typeid(int8_t)) {
      fprintf(stderr, "message has %d, not int8 data, not setting pixmap!\n", _msg->data_type);
      return;
    }
    else if (_msg->data_type < 0 || _msg->data_type > FRSM_PIXEL_MAP_T_TYPE_UINT8) {
...
{
  const frsm_pixel_map_t * msg = get_pixel_map_t(utime);
  int sz = frsm_pixel_map_t_encoded_size(msg);
  char * buf = (char *) malloc(sz * sizeof(char));
  frsm_pixel_map_t_encode(buf, 0, sz, msg);
...
{
  frsm_pixel_map_t * tmpmsg = load_pixel_map_t_from_file(name);
  set_from_pixel_map_t(tmpmsg);
  frsm_pixel_map_t_destroy(tmpmsg);
}

frsm_pixel_map_t * load_pixel_map_t_from_file(const std::string & name)
{
  ...
  ifs.close();
  frsm_pixel_map_t * ret_msg = (frsm_pixel_map_t *) calloc(1, sizeof(frsm_pixel_map_t));
  if (frsm_pixel_map_t_decode(tmpdata, 0, sz, ret_msg) < 0) {
